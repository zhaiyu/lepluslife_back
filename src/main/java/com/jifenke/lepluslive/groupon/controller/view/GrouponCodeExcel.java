...
public class GrouponCodeExcel extends AbstractExcelView {
    ...
    public void setExcelRows(HSSFSheet excelSheet, List<GrouponCode> orderList) {
        ...
        for (GrouponCode code : orderList) {
            ...
            // 积分客分润金额
            excelRow.createCell(9).setCellValue((code.getCommission() - (code.getShareToLockMerchant() + code.getShareToLockPartner() + code.getShareToTradePartner() + code.getShareToLockPartnerManager() + code.getShareToTradePartnerManager())) / 100.0);
            //  核销门店
            if (code.getState() == 1 || code.getState() == 2 || code.getState() == 3) {
