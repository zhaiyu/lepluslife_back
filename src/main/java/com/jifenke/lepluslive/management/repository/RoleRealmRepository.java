...
public interface RoleRealmRepository extends JpaRepository<Role_realm, Long> {
  ...
  @Query(value = "SELECT role_percode FROM  role_realm WHERE roleid=?1", nativeQuery = true)
  List<String> findRealmByID(Long id);

  @Query(value = "DELETE  FROM role_realm  WHERE roleid =?1", nativeQuery = true)
  void deleteRoleMenuByRoleID(Long id);


  @Query(value = "SELECT * FROM  role_realm WHERE roleid=?1", nativeQuery = true)
